...
public interface update_userMapper {
	...
	@Select("select u.*,p.position_name,c.Classname from lab_user as u left join lab_position as p on u.position_id=p.id left join lab_class as c on u.class_id=c.id limit #{offset},#{pageSize}")
